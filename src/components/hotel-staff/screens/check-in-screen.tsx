...
import { ArrowLeft, Package, User, Calendar, QrCode, Camera, ImageIcon, AlertTriangle, CheckCircle2 } from "lucide-react"
...
export function CheckInScreen({ order, onPhotoCaptured, onFlagIssue, onBack }: CheckInScreenProps) {
  ...
  const handleQRScan = (result: string) => {
    ...
    setScannedId(result)
    if (result === order.id || result.startsWith("BX-") || result.startsWith("bondex://")) {
      setQrVerified(true)
    } else {
            ...
            <div className="flex gap-2">
              {order.travelerPhotos.map((photo, i) => (
                <div key={i} className="w-20 h-20 rounded-lg bg-muted overflow-hidden">
                  <img src={photo || "/placeholder.svg"} alt={`Traveler photo ${i + 1}`} className="w-full h-full object-cover" />
                </div>
              ))}
            </div>
          </div>
